...
class TrainPipeline:
    def __init__(
        ...
        # Create unified post-transform components
        ...
        self.rasterizer = Rasterize([int(i // self.super_params.pixdim[0] * self.super_params.upscale_ratio) for i in self.super_params.crop_window_size]) # tool for rasterizing mesh with upscaled resolution
    ...
    def _prepare_transform(self, keys, modal, training_phase: str, **kwargs):
        ...
        valid_transform = pre_transform(
            ...
            self.super_params.crop_window_size,
            self.super_params.pixdim, phase=training_phase, 
            upscale_ratio=self.super_params.upscale_ratio, **kwargs # Use training_phase here by default
            )
    ...
    def _generate_downsampled_gt(self, seg_true, modal, decoder_size=False):
        ...
        This replaces the need to store downsampled ground truth in memory during data loading.
        ...
        """
        # Use the post-transform pipeline to generate downsampled ground truth
        # The _memory_efficient_post_transform now handles both 4D and 5D tensors properly
        ...
        # Return tensor directly (no indexing needed since single input returns tensor directly)
        return result
    ...
    def _memory_efficient_post_transform(self, seg_pred_list, seg_true_list, modal, to_gpu=True, decoder_size=False):
        ...
        if len(processed_preds) == 1:
            # Single tensor - return directly without adding extra dimension
            return processed_preds[0]
        elif processed_preds and hasattr(processed_preds[0], 'dim'):
            # Multiple tensors - stack them
    ...
    def _apply_resnet_padding(self, tensor):
        ...
        # The new UpscalingResNet has encoder layers that downsample
    ...
    def train_iter(self, epoch, phase, commit_log=True):
        ...
        if phase == "unet":
            ...
            # Add losses to the dictionary using wandb sections - semantic grouping
            ...
            if self.ct_train_loader is not None:
                for step, data_ct in enumerate(self.ct_train_loader):
                    ...
                    with torch.autocast(device_type=DEVICE):
                        ...
                        # Use decoder-sized post-transform to directly get upscaled size
                        seg_pred_ct_ds_decoder_size = self._memory_efficient_post_transform(seg_pred_ct, seg_true_ct, "ct", to_gpu=True, decoder_size=True)
                        
                        # Generate downsampled ground truth at decoder size on-the-fly
                        seg_true_ct_ds_decoder_size = torch.stack([
                            self._generate_downsampled_gt(seg_true_item, "ct", decoder_size=True)
                            for seg_true_item in seg_true_ct
                        ])
                        
                        # Also get regular size for decoder input (downscaled)
                        seg_pred_ct_ds = self._memory_efficient_post_transform(seg_pred_ct, seg_true_ct, "ct", to_gpu=True, decoder_size=False)
                        
                        # Calculate binary mask and compute distance map on decoder-sized data
                        ...
                        # Apply padding for ResNet compatibility on downscaled input
                        seg_pred_ct_ds_padded, pad_info = self._apply_resnet_padding(seg_pred_ct_ds)
                        
                        # Apply ResNet with padding - decoder will upscale the output
                        resnet_output_padded = self.decoder(seg_pred_ct_ds_padded)
                        
                        # Remove padding from ResNet output (handles upscaling)
                        resnet_output = self._remove_resnet_padding(resnet_output_padded, pad_info)
                        
                        # Apply the refinement at decoder size (no upscaling needed)
            ...
            # ResNet loss logging using wandb sections
            wandb.log({
            ...
            if self.ct_train_loader is not None:
                for step, data_ct in enumerate(self.ct_train_loader):
                    ...
                    # Generate downsampled ground truth for mesh extraction
                    seg_true_ct_ds = torch.stack([
                        self._generate_downsampled_gt(seg_true_item, "ct", decoder_size=False)
                        for seg_true_item in seg_true_ct
                    ])
                    # Generate ground truth mesh for myocardium only (label=2)
                    mesh_true_ct = self.surface_extractor(seg_true_ct_ds.to(DEVICE), labels=2)
                ...
                # GSN logging using wandb sections
                wandb.log({
    ...
    def valid(self, epoch, save_on):
        ...
        with torch.no_grad():
            for step, data in enumerate(valid_loader):
                ...
                # For CT, seg_true is assumed to be (B, C, D, H, W)
                
                # Generate downsampled ground truth at decoder size to match voxeld_mesh
                seg_true_ds = torch.stack([
                    self._generate_downsampled_gt(seg_true_item, modal, decoder_size=True)
                ...
                # Use decoder-sized post-transform to directly get upscaled size
                seg_pred_ds_decoder_size = self._memory_efficient_post_transform(seg_pred, seg_true, modal, to_gpu=True, decoder_size=True)
                
                # Also get regular size for decoder input (downscaled)
                seg_pred_ds = self._memory_efficient_post_transform(seg_pred, seg_true, modal, to_gpu=True, decoder_size=False)
                ...
                # Apply decoder at regular size and combine with decoder-sized prediction
        ...
        # Full network validation using wandb sections
        log_data_valid["validation/mesh_dice"] = eval_score_epoch
        log_data_valid["validation/df_mse"] = df_score_epoch
        ...
        if eval_score_epoch > self.best_eval_score:
            ...
            log_data_valid["validation/seg_true_vs_mesh_pred"] = wandb.Image(f"{visualization_dir}/seg_true_ds_vs_mesh_pred_epoch{epoch}.png")
            ...
            log_data_valid["validation/seg_true_vs_seg_pred"] = wandb.Image(f"{visualization_dir}/seg_true_ds_vs_seg_pred_ds_epoch{epoch}.png")
            ...
            log_data_valid["validation/template_vs_df_pred"] = wandb.Image(f"{visualization_dir}/template_vs_df_pred_epoch{epoch}.png")
            ...
            log_data_valid["validation/seg_true_vs_df_pred"] = wandb.Image(f"{visualization_dir}/seg_true_ds_vs_df_pred_epoch{epoch}.png")
            ...
            log_data_valid["validation/df_true_vs_pred"] = wandb.Image(f"{visualization_dir}/df_true_vs_pred.png")
    ...
    def test(self, save_on):
        ...
        with torch.no_grad():
            for step, data in enumerate(valid_loader):
                ...
                # Generate downsampled ground truth at decoder size to match voxeld_mesh
                seg_true_ds = self._generate_downsampled_gt(seg_true, modal, decoder_size=True)
    ...
    def ablation_study(self, save_on):
        ...
        for i, data in enumerate(valid_loader):
            ...
            # Apply standard decoder usage - in ablation study we keep things simple
        ...
        if is_unet_phase:
            ...
            if self.ct_valid_loader is not None:
                ...
                for step, data_ct in enumerate(self.ct_valid_loader):
                    ...
                    if dice_scores.shape[1] >= 1:  # Ensure we have at least 1 class (LV)
                ...
                if ct_dice_scores["lv"]:
                    ct_avg_dice = {}
                    for k, v in ct_dice_scores.items():
                        if v:  # Check if list is not empty
                            mean_val = np.mean(v)
                            if not np.isnan(mean_val):  # Check if mean is not NaN
                                ct_avg_dice[k] = mean_val
                    
                    # UNet validation using wandb sections
                    log_data_validation.update({
                        f"unet/val_ct_dice_{k}": v for k, v in ct_avg_dice.items()
                    })
            ...
            if self.mr_valid_loader is not None:
                ...
                for step, data_mr in enumerate(self.mr_valid_loader):
                    ...
                    # Debug: Check if we have any data left after filtering
                    if img_mr.shape[0] == 0:
                        print(f"Warning: No labeled slices found in MR batch {step}")
                        continue
                    ...
                    # Debug: Print dice scores shape and values for MR
                    if step == 0:  # Only print for first batch to avoid spam
                        print(f"MR Dice scores shape: {dice_scores.shape}, values: {dice_scores}")
                        print(f"MR Pred shape: {seg_pred_onehot.shape}, True shape: {seg_true_onehot.shape}")
                    
                    if dice_scores.shape[1] >= 1:  # Ensure we have at least 1 class (LV)
                        lv_dice = dice_scores[0, 0].item()
                        if not np.isnan(lv_dice):
                            mr_dice_scores["lv"].append(lv_dice)
                        if dice_scores.shape[1] >= 2:
                            myo_dice = dice_scores[0, 1].item()
                            if not np.isnan(myo_dice):
                                mr_dice_scores["myo"].append(myo_dice)
                        if dice_scores.shape[1] >= 3:
                            rv_dice = dice_scores[0, 2].item()
                            if not np.isnan(rv_dice):
                                mr_dice_scores["rv"].append(rv_dice)
                
                # Log MR validation metrics
                if mr_dice_scores["lv"]:
                    mr_avg_dice = {}
                    for k, v in mr_dice_scores.items():
                        if v:  # Check if list is not empty
                            mean_val = np.mean(v)
                            if not np.isnan(mean_val):  # Check if mean is not NaN
                                mr_avg_dice[k] = mean_val
                    
                    # UNet validation using wandb sections
                    log_data_validation.update({
                        f"unet/val_mr_dice_{k}": v for k, v in mr_avg_dice.items()
                    })
                    print(f"MR Dice Scores - LV: {mr_avg_dice.get('lv', 0):.4f}, "
                          f"MYO: {mr_avg_dice.get('myo', 0):.4f}, RV: {mr_avg_dice.get('rv', 0):.4f}")
        
        else:
            # ResNet phase: Validate only CT data with ResNet refinement
            # No ResNet validation for MR data (save_on == "cap")
            if save_on == "sct" and self.ct_valid_loader is not None:
                ...
                for step, data_ct in enumerate(self.ct_valid_loader):
                    ...
                    # Use decoder-sized post-transform to get upscaled version for ResNet compatibility
                    seg_pred_ct_ds_decoder_size = self._memory_efficient_post_transform(seg_pred_ct, seg_true_ct, "ct", to_gpu=True, decoder_size=True)
                    
                    # Also get regular size for decoder input
                    seg_pred_ct_ds = self._memory_efficient_post_transform(seg_pred_ct, seg_true_ct, "ct", to_gpu=True, decoder_size=False)
                    
                    # Apply ResNet refinement - use decoder-sized data for mask calculation
                    binary_mask_pred = (torch.argmax(seg_pred_ct_ds_decoder_size, dim=1, keepdim=True) == 0)
                    ...
                    # Apply decoder at regular size and combine with decoder-sized prediction
                    decoder_output = self.decoder(seg_pred_ct_ds)
                    seg_pred_ct_refined = seg_pred_ct_ds_decoder_size + mask * decoder_output
                    
                    # Generate downsampled ground truth for comparison at decoder size
                    seg_true_ct_ds = torch.stack([
                        self._generate_downsampled_gt(seg_true_item, "ct", decoder_size=True)
                        for seg_true_item in seg_true_ct
                    ])
                ...
                if ct_dice_scores["lv"]:
                    ...
                    # ResNet validation using wandb sections
                    log_data_validation.update({
                        f"resnet/val_ct_dice_{k}": v for k, v in ct_avg_dice.items()
                    })
                    print(f"CT Dice Scores (ResNet refined) - LV: {ct_avg_dice.get('lv', 0):.4f}, "
                          f"MYO: {ct_avg_dice.get('myo', 0):.4f}, RV: {ct_avg_dice.get('rv', 0):.4f}")
            
            # No ResNet validation for MR data - ResNet is only trained on CT data
            elif save_on == "cap":
                print(f"Skipping ResNet validation for MR data (save_on={save_on}) - ResNet only trained on CT data")
        
        # Log all validation metrics
        if log_data_validation:
            wandb.log(log_data_validation, step=epoch + 1, commit=commit)
        
        # Save NIfTI files at the last epoch of UNet training phase for MR validation dataset
        if is_unet_phase and epoch == self.super_params.pretrain_epochs - 1 and self.mr_valid_loader is not None:
            print(f"\n*** SAVING NIFTI FILES AT LAST UNET EPOCH ***")
            
            # Set up NIfTI save directory under visualization folder
            nifti_save_dir = f"{self.ckpt_dir}/visualizations/unet_final_epoch_nifti"
            os.makedirs(nifti_save_dir, exist_ok=True)
            
            self.encoder_mr.eval()
            with torch.no_grad():
                for step, data_mr in enumerate(self.mr_valid_loader):
                    # Extract case ID using the same method as in test function
                    case_id = os.path.basename(self.mr_valid_loader.dataset.data[step]["mr_label"]).replace(".nii.gz", '').replace(".seg.nrrd", '')
                    case_id = case_id.split('-')[0]  # For MR data
                    
                    # Create case-specific subfolder
                    case_dir = os.path.join(nifti_save_dir, case_id)
                    ...
                    # Filter out slices without labels for MR
                    img_mr_filtered, seg_true_mr_filtered = self._filter_unlabeled_slices(img_mr, seg_true_mr)
                    
                    # Skip if no labeled slices
                    if img_mr_filtered.shape[0] == 0:
                        print(f"Warning: No labeled slices found for case {case_id}, skipping NIfTI save")
                        continue
                    ...
                    seg_pred_mr_discrete = torch.argmax(seg_pred_mr, dim=1, keepdim=True)
                    
                    # Extract only the first frame from (2*D, C=1, H, W) tensors
                    # Assuming first D slices are the first frame
                    num_slices_per_frame = img_mr_filtered.shape[0] // 2
                    
                    # Take only the first frame (first D slices)
                    img_mr_first_frame = img_mr_filtered[:num_slices_per_frame, :, :, :]  # (D, C=1, H, W)
                    seg_true_mr_first_frame = seg_true_mr_filtered[:num_slices_per_frame, :, :, :]  # (D, C=1, H, W)
                    seg_pred_mr_first_frame = seg_pred_mr_discrete[:num_slices_per_frame, :, :, :]  # (D, C=1, H, W)
                    
                    # Convert 4D tensors (D, C=1, H, W) to 3D arrays (H, W, D) for NIfTI saving
                    # Original MR image (first frame only)
                    img_mr_3d = img_mr_first_frame[:, 0, :, :].permute(1, 2, 0).cpu().numpy()  # (H, W, D)
                    
                    # Ground truth segmentation (first frame only)
                    seg_true_mr_3d = seg_true_mr_first_frame[:, 0, :, :].permute(1, 2, 0).cpu().numpy()  # (H, W, D)
                    
                    # UNet prediction (first frame only)
                    seg_pred_mr_3d = seg_pred_mr_first_frame[:, 0, :, :].permute(1, 2, 0).cpu().numpy()  # (H, W, D)
                    
                    # Save as NIfTI files with identity affine matrix
                    affine = np.eye(4)
                    
                    # Save MR image
                    mr_img_nifti = nib.Nifti1Image(img_mr_3d, affine)
                    nib.save(mr_img_nifti, os.path.join(case_dir, f"{case_id}_mr_image.nii.gz"))
                    
                    # Save ground truth segmentation
                    gt_seg_nifti = nib.Nifti1Image(seg_true_mr_3d.astype(np.int16), affine)
                    nib.save(gt_seg_nifti, os.path.join(case_dir, f"{case_id}_ground_truth.nii.gz"))
                    
                    # Save UNet prediction
                    pred_seg_nifti = nib.Nifti1Image(seg_pred_mr_3d.astype(np.int16), affine)
                    nib.save(pred_seg_nifti, os.path.join(case_dir, f"{case_id}_unet_prediction.nii.gz"))
                    
                    print(f"Saved NIfTI files for case {case_id} in {case_dir}")
            
            print(f"*** NIFTI FILES SAVED TO: {nifti_save_dir} ***")
