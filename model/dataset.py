...
class Dataset(Randomizable, CacheDataset):
    """
        :params 
            image_paths: sorted list of image file absolute paths
            label_paths: sorted list of label file absolute paths, paired with image file

            slice_info_paths: sorted list of slice info file absolute paths, paired with image file

            keys: IDs of image, label and slice_info in a sequence, e.g., ('image', 'label', 'slice_info')
            section: expected data section, can be: `train`, `valid` or `test`.
            ...
            subset_frac: percentage of a subset to the whole dataset, default is 1.0.
            ...
            copy_cache: choose to copy it or not the data with deep_copy() in GPU, default is True.
    ...
    def __init__(
            ...
            num_workers: int = 0,
            slice_info_paths: Union[list, SupportsIndex]=None,
    ):
        ...
        if slice_info_paths is None:
            data = self._generate_data_list(image_paths, label_paths, keys)
        else:
            data = self._generate_data_list(image_paths, label_paths, keys, 
                                            slice_info_paths=slice_info_paths)
    ...
    def _generate_data_list(self, image_paths, label_paths, keys, **kwargs) -> list:
        datalist = load_multimodal_datalist(image_paths, label_paths, keys, 
                                            kwargs.get("slice_info_paths", None))
